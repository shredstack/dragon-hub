...
export type MergeResult =
  ...
  | {
      ok: false;
      reason: "invalid_token" | "no_target_account" | "same_account";
    };
...
export async function redeemAccountLink(token: string): Promise<MergeResult> {
  ...
  const [relayUser, targetUser] = await Promise.all([
    db.query.users.findFirst({
      where: eq(users.id, request.relayUserId),
      columns: { id: true, email: true, name: true },
    }),
    db.query.users.findFirst({
      where: eq(users.email, request.targetEmail),
      columns: { id: true, email: true, name: true },
    }),
  ]);

  if (!relayUser) return { ok: false, reason: "invalid_token" };
  if (!targetUser) return { ok: false, reason: "no_target_account" };
  ...
  await mergeRelayAccountInto(relayUser.id, targetUser.id);
  ...
  if (targetUser.email) {
    for (const link of [
      ...
      try {
        await link(targetUser.id, targetUser.email);
      } catch (error) {
  ...
  return { ok: true, userId: targetUser.id, email: targetUser.email! };
}
...
async function mergeRelayAccountInto(
  ...
  // The relay account is minutes old and now holds nothing. Every remaining FK
  // to it is cascade or set-null (see the comment above `users` in schema.ts),
  // so one statement resolves the graph — but it goes through the shared
  // deletion path anyway, so a relay account that somehow did claim a seat
  // releases it rather than stranding it.
  const { deleteUserAndReleaseSeats } = await import("@/lib/account-deletion");
