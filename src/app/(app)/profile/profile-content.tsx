...
export function ProfileContent() {
  ...
  /**
   * In the native shell this is the real camera action sheet
   * (`@capacitor/camera`), which is what exercises the Info.plist usage
   * strings; on the web it falls through to the hidden file input.
   */
  async function handlePickImage() {
    ...
    if (await isNativeCameraAvailable()) {
      ...
      if (photo) await uploadImageFile(photo.file);
      return;
    }
  ...
  return (
    <>
      ...
      {/* Last, and visually separated. Both stores require an in-app deletion
          path that is actually findable; burying it is its own rejection. */}
